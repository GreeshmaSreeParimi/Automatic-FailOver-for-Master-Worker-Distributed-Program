...
public class Client implements Watcher, Closeable {
     ...
     *
     * @param hostPort IP address:IP port ZooKeeper
     *  will listen at
     */
     ...
     * The session will be expired at 15 
     * seconds for no communication.
     */
    ...
    public void process( WatchedEvent e ) { 
        System.out.println("Inside process of main");
   ...
   /**
    ...
    public static void main( String args[] ) throws Exception {
        ...
        // create /tasks/task-000000000d (d=0-9) 
        client.createBagOfTasks( );

        // delete /workers and /tasks 
    ...
    private void createWorkerNode( ) throws Exception {
        ...
        System.out.println( "/workers created by client " 
            + pid );
    }
   

    /**
     * Creates the /tasks znode synchronously 
     * and thereafter submits
     * /tasks/task-000000000d where d=0-9.
     *  Each task has "submitted" as its data
     */
    ...
    /**
     * Launches a watcher for each task, 
     * confirms all tasks have been deleted,
     * deletes /tasks, checks all workers are gone, 
     * and finally deletes /workers.
     */
    private void confirmEmptyBag( ) throws Exception {
        ...
        /**
         * get all the tasks under /task
         */
        List<String> children
            = zk.getChildren( "/tasks", false);
        
        // sort them based on taskID
        Collections.sort(children);
        
        for ( int i = 0; i < nTasksSubmitted; i++ ) {
            // for each task, add watcher using taskID
            zk.exists( "/tasks/" + children.get(i),
                taskWatcher );
            System.out.println( "/tasks/" + children.get(i) 
                + " under watch" );
        }

        /** 
         * nTasksCompleted is incremented by each task watcher, 
         * so that the main thread waits until
         *  nTasksCompleted comes up to nTasksSubmitted.
         * */
        while ( nTasksCompleted < nTasksSubmitted )
            Thread.sleep( 1000 );

        // All tasks have been processed and deleted. 
        // So, it's time to delete /tasks.
        System.out.println( "all tasks deleted" );
        zk.delete( "/tasks", 0 );

        // Waits untill all workers disappeared. 
        // Then, we can delete /workers.
        while ( true ) {
            List<String> workers = 
                zk.getChildren( "/workers", false, null );
    ...
    /**
     * Watches any changes of /task and 
     * increments nTaskCompleted if the 
     * change was a task-deleting event. 
     * Otherwise, this method reschedules
     * a task watcher for detecting future events.
     */
    Watcher taskWatcher = new Watcher( ) {
        public void process( WatchedEvent event ) {
            System.out.println("Inside process of task watcher");
