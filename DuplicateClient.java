...
public class Client implements Watcher, Closeable {
   ...
   /**
     ...
     */
    private void deleteWorkersNode() throws Exception {
        ...
        zk.delete("/workers", -1); // Delete the "/workers" node
        System.out.println("/workers deleted");
    }
    ...
    private void deleteTasksNode() throws Exception {
        ...
        for (String child : children) {
            ...
            List<String> cds = zk.getChildren(childPath, false);
            for (String c : cds) {
                
                String cp = childPath + "/" + c;
                zk.delete(cp, -1); // Delete each child node
            }
